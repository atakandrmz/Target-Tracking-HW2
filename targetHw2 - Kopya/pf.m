...
measNoise = mvnrnd(muMeas,SigmaMeas,1000);
...
x = g(xi(i,1),v,xi(i,2),v,acc)
y = h(x(1), x(3), measNoise)
p = mvnpdf((y-[x(1) x(3)]),[],SigmaMeas)
pw = p*wi
pw = pw/(sum(pw))

muBarRe = [0 0 0 0]
for i =1:length(R) 
    muBarRe = muBarRe + pw(i).*g(xi(i,1),v,xi(i,2),v,acc)'
end

SigmaBarRe = zeros(4,4);

for i =1:length(R) 
    SigmaBarRe = SigmaBarRe + pw(i).*(g(xi(i,1),v,xi(i,2),v,acc)'-muBarRe)'*(g(xi(i,1),v,xi(i,2),v,acc)'-muBarRe)
end
...
error_ellipse(Sigma, mu,0.99)
error_ellipse([SigmaBar(1,1) 0;0 SigmaBar(3,3)], [muBar(1) muBar(3)],0.99,'style', '--')
error_ellipse([SigmaBarRe(1,1) 0;0 SigmaBarRe(3,3)], [muBarRe(1) muBarRe(3)],0.99,'style', '--')
