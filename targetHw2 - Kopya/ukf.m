...
% sigmaPtPos = R
% 
% xi = [sigmaPt0;sigmaPtPos]
% wi = 1/(length(R)+1);
% 
% muBar = [0 0]
% for i =1:length(R)+1 
%     muBar = muBar + wi.*g(xi(i,1),xi(i,2))'
% end
% 
% SigmaBar = [0 0;0 0];
% 
% for i =1:length(R)+1 
%     SigmaBar = SigmaBar + wi.*(g(xi(i,1),xi(i,2))'-muBar)'*(g(xi(i,1),xi(i,2))'-muBar)
% end
...
% error_ellipse(SigmaBar, muBar,0.99,'style', '--')
% 
% utPos = g(sigmaPt0(1,1),sigmaPt0(1,2))'
%     for i =1:length(R)
%         
%         utPosNew = g(sigmaPtPos(i,1),sigmaPtPos(i,2))'
